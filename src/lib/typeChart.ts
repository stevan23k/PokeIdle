...
export function getEffectiveness(attackType: string, defenseTypes: string[]): number {
  ...
  // If attack type is not found (e.g. struggle or custom typeless) return 1
  if (!TYPE_CHART[at]) return 1;
  
  let multiplier = 1;
  for (const dt of defenseTypes) {
    const defendType = dt.toLowerCase();
    if (TYPE_CHART[at][defendType] !== undefined) {
      multiplier *= TYPE_CHART[at][defendType];
    }
  }
  return multiplier;
}
