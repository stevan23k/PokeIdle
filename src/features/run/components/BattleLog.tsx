...
export function BattleLog() {
  ...
  return (
    <div className="h-44 border-2 border-border bg-surface-dark crt-screen relative flex flex-col overflow-hidden">
      ...
      >
        {logs.length === 0 && (
          <div className="text-center text-muted font-body text-[0.6rem] italic mt-2">
            Esperando eventos...
          </div>
        )}
        {logs.map((entry) => {
          ...
          switch (entry.type) {
            ...
            case "super":
              ...
              break;
            case "not-very":
              colorClass = "text-muted opacity-70";
              break;
            case "capture":
              colorClass =
          ...
          return (
            <div
              key={entry.id}
              className={clsx(
                "font-body text-[0.65rem] leading-snug break-words border-l-2 pl-2 border-transparent hover:bg-surface hover:border-border transition-colors py-0.5",
                colorClass,
              )}
            >
              {entry.type === "crit" ? (
